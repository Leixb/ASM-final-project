...
serie <- window(ts(read.table("./data/metro.dat"), start = 1996, freq = 12))

df <- tibble(passengers = serie, date = time(serie))
df %>%
...
serie_d12 <- diff(serie, lag = 12)
```

```{r 010-time-series, fig.cap="Time series of the data"}

df_diff <- tibble(passengers = serie_d12, date = time(serie_d12))
...
ggmonthplot(serie) + labs(y = "Thousands of Passengers")
```
...
ggAcf(serie_d12) + plot_spacer() + ggPacf(serie_d12) +
